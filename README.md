...
v1.0.0 使用标准化测试任务（README 添加徽章）在三种模式下进行了基准测试。
...
| 模式 | 总成本 | 输入 Token | 输出 Token | Agent 利用率 |
|------|-------|-----------|-----------|-------------|
| 单 Agent | $0.43 | 12,847 | 4,012 | 100%（单一） |
| TDD | $1.06 | 28,491 | 11,246 | 33%（每 Agent） |
| Swarms | $1.41 | 37,892 | 15,023 | 20%（每 Agent） |

### 测试用例

| 模式 | 编写测试 | 测试通过率 | 代码质量通过率 |
|------|---------|-----------|--------------|
| 单 Agent | 否 | -- | 50% |
...
- **单 Agent** 适合快速原型和简单任务，成本最低但质量最差
- **TDD** 是生产环境的推荐选择，质量与成本的最佳平衡
- **Swarms** 适合大型复杂任务，质量最高但成本也最高

性能基准测试文件位于 `architectures/dev-test-loop/workflow/bench-*.yaml`。
