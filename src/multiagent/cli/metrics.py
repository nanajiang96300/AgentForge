"""
CLI for metrics — token/cost statistics.

Usage:
    multiagent metrics                       Summary for all agents
    multiagent metrics --agent pm            Filter by agent
    multiagent metrics --task-id <id>        Filter by task
    multiagent metrics --json                JSON output
"""
...
def cmd_metrics(db=None, agent=None, task_id=None, json_output=False, details=False):
    ...
    MetricsRepository manages its own connection.
    """
    _close_db = False
    if db is None:
