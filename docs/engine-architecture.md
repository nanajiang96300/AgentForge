...
- **任务**: README 添加徽章（标准测试用例）
- **模型**: deepseek-v4-pro (PM/Dev/Architect), deepseek-v4-flash (Test/Conductor)
- **运行次数**: 每种模式 3 次取均值

### 执行时间

| 模式 | 执行时间 | Agent 调用次数 | 质量评分 |
|------|---------|---------------|---------|
| 单 Agent | 108s | 1 | 5/10 |
| TDD (PM+Dev+Test) | 241s | 3 | 9/10 |
| Swarms (多 Dev 并行) | 330s | 3-5 | 10/10 |

### Token 消耗

| 模式 | 总成本 | 输入 Token | 输出 Token |
|------|-------|-----------|-----------|
| 单 Agent | $0.43 | 12,847 | 4,012 |
| TDD | $1.06 | 28,491 | 11,246 |
| Swarms | $1.41 | 37,892 | 15,023 |
...
|------|---------|-----------|---------|
| 单 Agent | 否 | N/A | 50% |
| TDD | 是 (Test Agent) | 100% | 90% |
| Swarms | 是 (多视角) | 100% | 95% |

### 结论

1. **单 Agent** 适合快速原型，成本最低 ($0.43) 但质量最差 (5/10)
2. **TDD** 是生产默认推荐，成本适中 ($1.06) 质量高 (9/10)
3. **Swarms** 适合大型复杂任务，质量最高 (10/10) 但成本也最高 ($1.41)
4. 多 Agent 协作的额外 Token 开销 (~2.2x) 换来质量提升 (~1.8x)
5. TDD 模式比 Traditional 模式多 ~15% Token 消耗但测试覆盖从 0 提升到 100%

> 详细基准测试配置文件见 `architectures/dev-test-loop/workflow/bench-*.yaml`
