...
> 版本：v0.7.5 | 最后更新：2026-06-23 | 覆盖范围：Phase 1 ~ Phase 7
...
| **v0.7.5** | 7 | 角色系统 CLI, Workflow CLI + 验证引擎, Dashboard 重写 (Jinja2), Designer V2 (边类型), 多拓扑测试 |
...
├── __init__.py
├── db.py                       # StateDB — SQLite WAL + 6 表 + 线程安全 + data retention
├── engine.py                   # AgentSpawner — 进程组管理 (killpg) + metrics
├── engine_cli.py               # multiagent run CLI
├── orchestrator.py             # WorkflowOrchestrator — DAG 编排 + 3 层重试硬顶
├── conductor.py                # Conductor — 调度守护进程 + 孤儿恢复
├── conductor_cli.py            # Conductor CLI — double-fork daemon
├── pm_cli.py                   # 统一命令分发
├── metrics_cli.py              # Metrics CLI
├── metrics.py                  # 指标聚合/导出
├── notify.py                   # Discord 通知 + i18n + retry + debounce
├── dashboard.py                # Flask 路由 + API (纯 Python, 395 行)
├── pm_discover.py              # GitHub Issues 自动发现
├── role_cli.py                 # [v0.7.5] 角色管理 CLI
├── workflow_cli.py             # [v0.7.5] 工作流 CLI + 验证引擎
├── interfaces.py               # ABC 抽象接口
├── templates/                  # [v0.7.5] Jinja2 模板
...
│   ├── claude_code.py          # ClaudeCodeAdapter (model/memory/permissions)
...
│   └── progress.py             # 进度计算
├── runtime/                    # 运行时
│   └── registry.py             # AgentRegistry + personality 字段
├── services/                   # 服务
│   ├── checkpoint.py           # 检查点管理
...
│   ├── dev/SKILL.md
│   └── test/SKILL.md
├── prompts/                    # Prompt 模板 (few-shot)
│   ├── pm.md
│   ├── dev.md
│   └── test.md
└── templates/                  # [v0.7.5] 骨架模板
    ├── SKILL.md.skeleton       # 角色 SKILL.md 骨架
    └── prompt.md.skeleton      # 角色 prompt.md 骨架

gates/                          # 门禁测试 (20 模块, ~200 tests)
├── regression.py               # 总入口
├── test_db.py                  # 5 tests
...
├── test_retry_cap.py           # [v0.7.0] 5 tests
├── test_hooks.py               # [v0.7.0] 4 tests
├── test_dashboard.py           # [v0.7.5] 20 tests
├── test_notify.py              # [v0.7.5] 17 tests
├── test_db_cleanup.py          # [v0.7.5] 9 tests
├── test_engine_process.py      # [v0.7.5] 6 tests
├── test_graph_engine.py        # [v0.7.5] 21 tests
├── test_agent_registry.py      # [v0.7.5] 12 tests
├── test_pm_search.py           # [v0.7.5] 7 tests
├── test_workflow_cli.py        # [v0.7.5] 20 tests
└── test_topology.py            # [v0.7.5] 18 tests
└── workflows/                  # [v0.7.5] 测试工作流 YAML
...
1. **WebSocket Dashboard** — 实时推送替代 15s 轮询
2. **多运行时适配** — OpenCode 适配器完善
3. **Systemd 集成** — systemd unit 模板、journald 日志
4. **Metrics 告警** — Token 超限/Agent 超时自动告警
5. **Slack / 飞书通知** — 多平台通知支持
6. **Workflow 版本管理** — 工作流历史版本对比与回滚
